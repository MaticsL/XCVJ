...
def get_result():
        ...
        while i <= 15: # wait 60s
            i += 1
            time.sleep(4)
            url = "http://www.bnuoj.com/v3/ajax/status_data.php?bSearchable_0=true&sSearch_0=e1e2e"
...
while True:
        ...
        try:
	      ...
	      db.close()
           ...
           for row in results:
              ...
              user = row[4]
           print "----Run ID=",runid,"----"
           sql = "UPDATE virtualsolution SET status='Judging' WHERE runid='%d'" % (runid)
           cursor.execute(sql)
           db.commit()
	   ...
	   db.close()
        ...
        # Update DB
        sql = "UPDATE virtualsolution SET status='%s' WHERE runid='%d'" % (ret,runid)
        try:
           cursor.execute(sql)
           db.commit()
           print "----finish----"
